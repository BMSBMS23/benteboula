...
if(isset($_GET['deletes'])){
    $id = $_GET['deletes'];
    $delete= mysqli_query($conn,"DELETE FROM salle WHERE idsalle = '$id'");
    if($delete){
    ...
    <section class="container" style="width: 80%;margin-left:7%;margin-top:1%;">
        <div class="row">
            <div class="col-lg-8 col-sm" style="margin-left:10%; margin-bottom:2%;">
                <h1 class="fs-2 text-center lead text-primary" style="font-family:Verdana, Geneva, Tahoma, sans-serif">Gestion des Salles</h1>
            </div>
        </div>
        <div class="dropdown-divider" style="margin-bottom: 15px; margin-right: 8%; border-top:1px solid #ccccd0;"></div>
        <div class="row">
            <div class="col-md-4">
                <h5 class="fw-bold mb-0"> La liste des Salles</h5>
            </div>
            <div class="col-md-7">
        ...
        <div class="row">
            ...
            <table class="table">
                <thead class="table-light">
                  ...
                  <tr>
                    <th scope="col" style="width:25%;text-align: start;">ID</th>
                    <th scope="col" style="width:25%;text-align: start;">NOM DU SALLE</th>
                    <th scope="col" style="width:25%;text-align: start;">NUMBER DE PLACE</th>
                ...
                <tbody>
                    <?php
                        while ($row = mysqli_fetch_assoc($result)) {
                            ?>
                  <tr>
                        <td style=" text-align: start;"><?=$row['idsalle']?></td>
                        <td style=" text-align: start;"><?=$row['nomsalle']?></td>
                        <td style=" text-align: start;"><?=$row['nbrplace']?></td>
                            
                    <td>
                      
                        <a href="edits.php?edits=<?=$row['idsalle']?>" class="text-primary me-2 editbtn"title="Modifier"><i class="fas fa-edit"></i></a>
                     
                        <a href="salle.php?deletes=<?php echo $row['idsalle']?>" onclick="return confirm('Voulez-vous supprimer cette salle ?')" class="text-danger me-2 deletebtn"title="supprimer"><i class="fas fa-trash-alt"></i></a>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="createModalLabel">Nouveau salle</h1>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          
            <form action="" method="post" autocomplete="off" id="add" >
            <input type="hidden" id="action" value="addsalle" >
                <div class="mb-3">
                  <label for="" class="form-label">Nom du salle</label>
                  <input type="text" class="form-control" id="nomsalle" required="">
                </div>
                  <div class="mb-3">
                    <label for="" class="form-label">Nombre du place</label>
                    <input type="number" class="form-control" id="nbrplace" required="">
                  </div>
